...
def count_good_cds(cds_features, ref_faidx, target_faidx, ref_children, target_sub_features, feature_list):
    ...
    for cds_group in grouped_cds:
        ...
        longest_ORF, longest_ORF_coords = get_longest_ORF(cds_seq, cds_group)
...
def get_longest_ORF(cds_seq, cds_group):
    ...
    if cds_group[0].strand == "-":
        cds_length = len(cds_seq)
        longest_ORF = [cds_length - longest_ORF[1], cds_length - longest_ORF[0]]
    return longest_ORF_seq, longest_ORF
