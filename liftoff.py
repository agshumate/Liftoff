...
def parse_args():
    ...
    parser.add_argument('-t', required=True, help="target fasta genome to lift genes to")
    parser.add_argument('-r', required=True, help = "reference fasta genome to lift genes from")
    group.add_argument('-g', help="annotation file to lift over in gff or gtf format")
    ...
    parser.add_argument('-p', required=False, help= "processes", default=1)
    ...
    parser.add_argument('-u', required=False, help= "name of file to write unmapped features to", default="unmapped_features")
    ...
    parser.add_argument('-unplaced', required=False, help="text file with name(s) of unplaced sequences to map genes from after genes from chromosomes in chroms.txt are mapped"  )
    parser.add_argument('-a', required=False, help="minimum alignment coverage to consider a feature mapped [0-1]", default=0.5)
    parser.add_argument('-copies', required=False, action='store_true', help="look for extra gene copies in the target genome")
    parser.add_argument('-s', required=False, help="minimum sequence identity to consider a feature mapped [0-1]", default=0.5)
    parser.add_argument('-sc', required=False, help='with -copies, minimum sequence identity for which a gene is considered a copy. Must be greater than -s', default = 1.0 )
    parser.add_argument('-m', required=False, help="Minimap2 path")
    parser.add_argument('-dir', required=False, help="name of directory to save intermediate fasta and SAM files", default="intermediate_files")
   ...
   #parse args
    ...
    if len(unmapped_features) > 0 and target_chroms[0] != target_fasta:
       ...
       unmapped_features = liftover_types.map_unmapped_genes_agaisnt_all(unmapped_features, target_fasta,
                                                                         ...
                                                                         children_features, parent_order, minimap2_path,inter_files)
