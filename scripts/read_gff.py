...
def match_CDS_to_exon(CDS_start, CDS_end, gene_list, log):
    ...
    if parent_exon is None:
        log.write("No parent Exon found for CDS at: " + CDS_start + "-" + CDS_end + "\n")
    else:
        return parent_exon


def add_transcript(gene_list, source, start, end, strand, desc, log):
    tran_name = (desc.split("ID=")[1]).split(";")[0]
    if "Parent=" in desc:
        parent_name = (desc.split("Parent=")[1]).split(";")[0]
    elif "geneID" in desc:
        parent_name = (desc.split("geneID=")[1]).split(";")[0]
    if parent_name in gene_list.keys():
        ...
        log.write("No parent gene for transcript: " + tran_name + "\n")
...
def get_genes_to_lift(file, warnings_file):
    ...
    for line in lines:
        if line[0] != "#": #skip header lines
            ...
            elif line[feature] == "transcript" or line[feature] == "mRNA":
                exon_id = 0
                add_transcript(gene_list, line[source], line[start], line[end], line[strand],  line[desc], log)
